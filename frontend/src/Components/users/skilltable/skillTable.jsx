...
const UserPage = ({ id }) => {
  ...
  const skillOptions = category
    ? skillStructure.find(({ category: c }) => c === category)?.skills
    : [];

  const subSkillOptions = skill
    ? skillOptions.find(({ skill: s }) => s === skill)?.subSkills
    : [];
  ...
  const removeSubSkill = async (index) => {
    fetch(
      apiurl +
        "/deleteuserskill?" +
        new URLSearchParams({ userskillId: index }),
      {
        method: "DELETE",
      }
    )
  ...
  useEffect(() => {
    ...
    if (id) {
      const url =
        apiurl + "/getallskillsofuser?" + new URLSearchParams({ userId: id });
              ...
              (
                ...
                <tr key={listId}>
                  ...
                  <td>
                    ...
                    <button onClick={() => delConfirmation({ index: listId })}>
