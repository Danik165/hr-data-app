...
export default function AddUserCertificate({ id, toggleForm }) {
  ...
  const validateInput = () => {
    ...
    if (
      new Date(newCertificate.Issue_date) >=
      new Date(newCertificate.Validity_date)
    ) {
      ...
      .then((response) => {
        if (response.redirected) {
          ...
          response.json().then((data) => {
            ...
            setNewCertificate({
              ...newCertificate,
              CertificateProvider: data.data[0].CertificateProvider,
            });
          });
        }
      })
      ...
      <form className="add-certificate-form">
        ...
        <select
          onChange={(e) =>
            setNewCertificate({
              ...newCertificate,
              CertificateProvider: e.target.value,
            })
          }
        >
        ...
        >
          ...
          {certificateList
            .filter(
              (obj) =>
                obj.CertificateProvider == newCertificate.CertificateProvider
            )
            .map((obj, i) => (
              <option key={obj.CertificateID} value={obj.CertificateID}>
                {obj.CertificateName}
              </option>
            ))}
        </select>
