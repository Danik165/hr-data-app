...
const AddSkillForm = () => {
  ...
  const fetchSkills = (category) => {
    fetch(
      apiurl +
        "/skillbycategory?" +
        new URLSearchParams({ categoryName: category })
    )
      .then((response) => {
        if (response.redirected) {
          window.location.replace(response.url);
        ...
        >
          <CDBCardBody className="mx-4">
            ...
            <select
              ...
              onChange={(e) => handleCategorySelecetion(e.target.value)}
            >
            ...
            {categoryEnabled && (
              <CDBInput
                ...
                onInput={(e) =>
                  setNewSubSkill({ ...newSubSkill, category: e.target.value })
                }
              />
            )}
            ...
            {skillEnabled && (
              <CDBInput
                ...
                iconClass="text-muted"
                onInput={(e) =>
                  setNewSubSkill({ ...newSubSkill, skill: e.target.value })
                }
              />
            )}
            <CDBInput
              ...
              iconClass="text-muted"
              onInput={(e) =>
                setNewSubSkill({ ...newSubSkill, subSkill: e.target.value })
              }
            />
