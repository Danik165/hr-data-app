    ...
    return (
      ...
      <CDBContainer>
        <CDBCard style={{ width: '30rem','border-radius':'0px 0px 10px 10px'  }}>
          ...
          <br />
          <select id="department" name="department" className='department-dropdown' onChange={e => {handleDeptSelection(e.target.value);setNewDepartment({...newDepartment,department_ID:e.target.value})}}>
          {departments.map(department => 
                <option id={department} value={department.DepartmentID}>{department.DepartmentName}</option>
          )}
          </select>

            {enabled && <CDBInput label="New Department" type="text" icon="user-friends" iconClass="text-muted" onChange={e => setNewDepartment({...newDepartment,department_Name:e.target.value})}/> }
           
            <CDBInput style={{'border-radius':'0px'}} label="Role" type="text" icon="lock" iconClass="text-muted" onChange={e => setNewDepartment({...newDepartment,roleName:e.target.value})}/>
            <div class='row mb-2 '>
