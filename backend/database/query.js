const sqlQuery = {
  ...
  insertNewUser:
    "Insert into users(UserID,Name,EmailID,RoleID,DepartmentID,AccessID) values (?,?,?,?,?,?)",
  ...
  selectDepartmentIdByName:
    "Select DepartmentID from department where DepartmentName=?",

  //Select Roles
  selectRoleIdByDepartmentIdandRoleName:
    "Select roleID from role where departmentID=? and RoleName = ?",
  selectRoleNameByDepartmentId:
    "Select roleID,RoleName from role where DepartmentID =? ",

  // OTP table update or select
  selectOtpbyUserIdandUniqueId:
    "Select otp from otp_request where UserId =? and uniqueId = ?",
  setOTPTransactionSuccess:
    "Update otp_request set success=? where uniqueId = ?",
  ...
  selectSkillsbyCategoryId:
    "SELECT SkillName FROM company_skills.skills where CategoryID=?",
  selectCategoryIdbyName:
    "Select CategoryID from category where CategoryName=?",
  selectCategoryIdbyCategoryName:
    "Select CategoryID from category where CategoryName = ?",
  selectSkillIdbySkillNameandCategoryId:
    "Select SkillID from skills where CategoryID = ? and SkillName = ?",
