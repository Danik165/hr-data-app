  ...
  for (let k in obj) {
    if (obj.hasOwnProperty(k)) {
      if (typeof obj[k] === "object") {
        ...
        typeof obj[k] == "string" &&
        obj[k].toLowerCase().search(val) >= 0 &&
        !DoNotMatchList.has(k)
      ) {
...
const Search = async ({ searchValue }) => {
  ...
  try {
    const [matchedProfiles] = await db
      .promise()
      .query("CALL SEARCH_ALL(?)", [searchParameter]);
    ...
    for (let i = 0; i < data.length; i++) {
      [skillDetails] = await db
        .promise()
        .query("CALL GET_SKILL_DETAILS_OF_USER(?)", [data[i].EmployeeID]);
...
const GetAllSkillDetailsofUser = async ({ id }) => {
  try {
    const [categoryDetails] = await db
      .promise()
      .query("CALL GET_COMPLETE_USER_SKILLS(?)", [id]);
    ...
    for (let i = 0; i < data.length; i++) {
      const [skillDetails] = await db
        .promise()
        .query("CALL GET_SUB_SKILLS_NAME(?)", [data[i].SubSkillIDList]);
...
const GetAllSkillSet = async () => {
  try {
    const [category] = await db
      .promise()
      .query("CALL GET_COMPLETE_SKILL_SET()");
    ...
    for (let i = 0; i < skills.length; i++) {
      ...
      for (let j = 0; j < skillList.length; j++) {
        [subskills] = await db
          .promise()
          .query("CALL GET_COMPLETE_SUB_SKILL_SET(?)", [skillList[j]]);
        if (subskills[0][0]) {
          tempObj.push({
            skill: skillList[j],
            subSkills: subskills[0][0].subSkills.split(","),
          });
        } else {
