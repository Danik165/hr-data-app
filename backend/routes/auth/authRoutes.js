...
router.get("/api/forgotpassword", async (req, res) => {
  ...
  try {
    const [rows] = await db
      .promise()
      .query(sqlQuery.selectUserIdByEmailId, [emaiId]);
    if (rows.length == 0) {
    ...
    await db
      .promise()
      .query(sqlQuery.insertNewOtpRequest, [userId, uniqueId, hashOtp, false]);
    ...
    const emailContent = {
      ...
      body:
        "You have raised a request to Reset Password. Use this OTP to change to your password \n" +
        otp +
    ...
    try {
      ...
      jwt.verify(token, jwtSecretKey, async (err, decodedToken) => {
        if (err) {
          ...
          const [rows] = await db
            .promise()
            .query(sqlQuery.selectOtpbyUserIdandUniqueId, [userId, uniqueId]);
          ...
          if (rows.length == 0) {
            ...
            if (result) {
              if (
                ...
                db.promise().query(sqlQuery.setOTPTransactionSuccess, [
                  1,
                  uniqueId,
                ]);
                res
